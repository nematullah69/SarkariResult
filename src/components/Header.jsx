...
import { Home, FileText, CreditCard, Info, GraduationCap, BookOpen, Key, Phone, Bell, Menu, X, Briefcase } from 'lucide-react';
...
const Header = () => {
  ...
  const getCurrentPageTitle = () => {
    const page = navigation.find(nav => nav.id === location.pathname);
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between py-4">
          ...
          <Link to="/" className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm">Government Examination Results & Information</p>
            </div>
          </Link>
        ...
        <nav className="hidden md:flex border-t border-blue-700">
          <div className="flex flex-wrap">
            {navigation.map((nav) => {
              const Icon = nav.icon;
              const isActive = location.pathname === nav.id || 
                             (nav.id !== '/' && location.pathname.startsWith(nav.id));
              return (
                <Link
                  key={nav.id}
                  to={nav.id}
